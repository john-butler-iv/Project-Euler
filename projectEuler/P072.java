...
public class P072 extends ParameterizedProblem<Integer> {
	...
	public long solve(Integer maxD, boolean printResults) {
		...
		// the number of reduced proper fractions for any denominator d is the number of
		// fractions which don't
		// reduce. The number of fractions that reduce are those that share a factor
		// with d, so what's left is
		...
		if (printResults)
			System.out.println("There are " + numFracs + " reduced proper fractions whose denominator is <= " + maxD);
		return numFracs;

...
public class P073 extends ParameterizedProblem<Integer> {
	...
	public long solve(Integer maxD, boolean printResults) {
		...
		// Fraction is a separate class from FractionPair because it's marginally faster
		// to copy one pointer than
		// two integers, a numerator and denomiator, assuming we're using each fraction
		// more than once.
		...
		while (!stack.isEmpty()) {
			...
			// num1 + num2 / den1 + den2 generates all desired fractions, see writeup for
		...
		if (printResults)
			System.out.println(
					"There are " + totalFracs + " fractions between 1/3 and 1/2 whose denominator is <= " + maxD);
		return totalFracs;
	}

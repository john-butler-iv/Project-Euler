...
public class P078 extends Problem {
	 ...
	 * by changing sign to an int, but that's unnecessary for our use case
	 */
	...
	/**
	 * Iterates through the gaps between the different "Fibonacci indices". Used by
	 * findFibAdds()
	 */
	...
	/**
	 * Iterates through the different indices which you need to add/sub together to
	 * find the next partition number. For example for the real Fiboanacci sequence,
	 * it would be {1, 2}, since you only add the last two terms. I should probably
	 * refactor this into a separate file, but I don't want to deal with two files.
	 * Also it only adds ~50 lines to keep here.
	 */
	private Iterator<FibInfo> findFibAdds() {
		return new Iterator<FibInfo>() {
			...
			public boolean hasNext() {
				// there is actaully a restriction from overflow, but to figure that out, I
				// would keep track of the
				// next return value and I don't want to do that.
				// Also I'm using this value to index in an array, and if it overflows, I'll run
				// out of memory
				return true;
			}
			...
			public FibInfo next() {
				...
				// we don't need to reset currSign. Even if there's an overflow, currSign & 2
				// will still
				// do it

...
public class P058 extends ParameterizedProblem<Double>{
	...
	public static void main(String[] args){
		new P058().solve(new P058().getDefaultParameter(), true);
	}
	@Override
	...
	public long solve(Double goalRatio, boolean printResults){
		...
		for(int sideLen = 3; true; sideLen += 2){
			...
			if(primes / (double) total < goalRatio){
				if(printResults)
					System.out.println(sideLen + " is the first side length whose ratio of diagonal primes is bellow " + (100 * goalRatio) + "%");
				return sideLen;
			}
		}
	}

...
public class P084 extends ParameterizedProblem<Integer> {
	...
	public static void main(String[] args) {
		Double[] rollProbs = figureRollProbs(6);
		System.out.println("Jail: " + ((int) (rollProbs[0] * 10000) / 100.0) + "%");
		for (int i = 1; i < rollProbs.length; i++)
			System.out.println(i + ": " + ((int) (rollProbs[i] * 10000) / 100.0) + "%");
		P084 p = new P084();
		p.solve(6, true);
	}
	...
	long solve(Integer parameter, boolean printResults) {
		...
		Double[] rollProbs = figureRollProbs(parameter);
	...
	private static Double[] figureRollProbs(int parameter) {
		return figureRollProbs(parameter, 3);
	}

	private static Double[] figureRollProbs(int parameter, int maxDepth) {
		if (maxDepth == 0)
			return new Double[] { 1.0 };

		double rollProb = 1.0 / (parameter * parameter);

		Double[] rollProbs = new Double[parameter * 2 * maxDepth];
		for (int i = 0; i < rollProbs.length; i++)
			rollProbs[i] = 0.0;

		for (int die1 = 1; die1 <= parameter; die1++) {
			for (int die2 = 1; die2 <= parameter; die2++) {
				if (die1 != die2) {
					rollProbs[die1 + die2] += rollProb;
				} else {
					Double[] recursiveProbs = figureRollProbs(parameter, maxDepth - 1);
